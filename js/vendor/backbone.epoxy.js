...
(function(root, factory) {
	...
	// -----------
	var modelMap;
	var modelSuper = superClass(Backbone.NestedModel);
  ...
  ////console.log("***json is"+JSON.stringify(json));
			...
			//We need to flatten this json...and add the flat values as attribute strings
...
////console.log("***json is"+JSON.stringify(json));
	...
	function readAccessor(accessor) {
		...
		////console.log("***Reading accessor func "+accessor());
			...
			////////console.log("***Reading accessor object "+accessor);
...
/** Get Unnested String...Utility function to generate unnested strings and it's arguments from a JSON Object
@param: object get object
**/

function getUnNestedStrings(prefix,object)
{
  ...
  ////console.log("@###  in source to view context"+ _.pairs(context)+ " " + options+ " " + name+" " + prefix); 
    ...
    ////console.log("Adding attribute"+attribute+ " " + source[attribute] + " " + _.isObject(value));
	...
	function accessViewDataAttribute(source, attribute, value, options) {
		...
		////////console.log("***In function accessViewDataAttribute" );	
	...
	////////console.log("####In function bindElementToView declaration is:"+declarations+ " context " +  JSON.stringify(context.$model()) );	
		...
		try {
			 //console.log("Checking nested declaration");
		//We need to check is a deep-nested element either an array or an object. In which case we add an element at run-time.
		var val=declarations.split(",")[0].split(":")[1];
		 ...
		 //console.log("bindins are "+_.pairs(bindings) );	
		 	
	if(bindings.value==undefined) {	 	
		 	if(val.indexOf(".")>-1 || val.indexOf("[")>-1)
		{
		//It's a nested attribute, we need to add this to the context
		 //console.log("In nested declaration");
		bindings.value = function(value) {
					return accessViewDataAttribute(context.$model(), val, value, {});
				};
				
				
		//console.log("**Nested declration" +val);
		}
		 ...
		 {
			 //console.log("The upper parserFunct gave an error" + val);
			 var bindings={};
				bindings.value = function(value) {
					return accessViewDataAttribute(context.$model(), val, value, {});
				};
				 //console.log("bindings is" + bindings);
		...
		////console.log("bindins are "+JSON.stringify(bindings) );	
		
		_.each(bindings, function(accessor, handlerName) {
                ////console.log("##handlerName is"+handlerName + " accssor is" + accessor );
